...
# Function to recreate the database by dropping al the data and creating it again
def recreate_db():
	db.drop_all()
	db.create_all()
	db.session.commit()

# Function to seed all the data in the recreated database
def seeder():
...
#Function which calls the above two functions together
def rsd():
